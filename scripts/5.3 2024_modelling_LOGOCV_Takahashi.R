...
#--------------------------------------------
brms::get_prior(bf(
  video_pce_IN ~ wiggleN_Takahashi, hu ~ wiggleN_Takahashi), 
  data = result_df,
...
prior <- c(
  ...
  set_prior("normal(1, 5)", class = "b"),                      # Prior for regression coefficients (mu_wiggleN_Takahashi)
  set_prior("normal(0, 2)", class = "b", dpar = "hu"),         # Prior for regression coefficients (hu_wiggleN_Takahashi)
...
m_Takahashi_prior <- brm(bf(
  ...
  prior = prior,
  sample_prior = "only", 
  file = "./brms/Takahashi/m_Takahashi_prior")

m_Takahashi_post <- brm(bf(
  video_pce_IN ~ wiggleN_Takahashi, hu ~ wiggleN_Takahashi), 
  data = result_df,
  family = hurdle_negbinomial(link = "identity", link_shape = "log", link_hu = "logit"),
  iter = 2000, warmup = 1000, chains = 4, cores = cores,
  prior = prior,
  sample_prior = T, 
  file = "./brms/Takahashi/m_Takahashi_post")

# Compare the empirical distribution of the data y to the distributions of simulated/replicated data yrep 
# from the posterior predictive distribution. 

# bayesplot::ppc_dens_overlay(y = result_df$video_pce_IN, yrep = m_Takahashi_pp[1:100, ]) +
#   labs(title = "Prior Predictive Distribution",
#        x = "video_pce_IN",
#        y = "Density")
performance::check_predictions(m_Takahashi_post) +  
...
m_Takahashi_pp <- posterior_predict(m_Takahashi_post)
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi_ppd.png"), PPD, width = 5, height = 5, dpi = 300)

loo_Takahashi_post = loo(m_Takahashi_post)
loo_Takahashi_post

#-----------------------------------------------------
# Extract parameter draws for the prior-only model
#-----------------------------------------------------
prior_draws <- as_draws_df(m_Takahashi_prior)

# Extract parameter draws for the posterior model
posterior_draws <- as_draws_df(m_Takahashi_post)
...
draws$Parameter <- recode(draws$Parameter,
                          ...
                          "b_wiggleN_Takahashi" = "mu_wiggleN_Takahashi",
                          "b_hu_Intercept" = "hu_Intercept",
                          "b_hu_wiggleN_Takahashi" = "hu_wiggleN_Takahashi",
                          "shape" = "Shape")
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi_prior_post.png"), PPD2, width = 5, height = 5, dpi = 300)

result_df %>%
  ungroup() %>% 
  modelr::data_grid(wiggleN_Takahashi = modelr::seq_range(wiggleN_Takahashi, n = 100)) %>%
  add_predicted_draws(m_Takahashi_post) %>%
  ggplot(aes(x = wiggleN_Takahashi, y = video_pce_IN)) +
  stat_lineribbon(aes(y = .prediction), .width = c(.90, .80, .50), alpha = 0.9) +
  geom_point(data = result_df) +
  scale_fill_brewer(palette = "Blues") +
  scale_color_brewer(palette = "Blues") + 
  labs(y = "Video PCE", x = 'Wiggle_Takahashi') +
...
# CE_mu_m_Takahashi = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.9)
# CE_mu_m_Takahashi
# CE_mu_h_m_Takahashi = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.9, dpar = "hu")
# CE_mu_h_m_Takahashi
...
# CE_pi_m_Takahashi = conditional_effects(m_Takahashi_post, method = "posterior_predict", prob = 0.9)
# CE_pi_m_Takahashi
...
# Extract components:
# --------------------------
CE50_mu = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.5)
CE50_mu_h = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.5, dpar = "hu")
CE50_pp = conditional_effects(m_Takahashi_post, method = "posterior_predict", prob = 0.5)

CE80_mu = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.8)
CE80_mu_h = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.8, dpar = "hu")
CE80_pp = conditional_effects(m_Takahashi_post, method = "posterior_predict", prob = 0.8)

CE90_mu = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.9)
CE90_mu_h = conditional_effects(m_Takahashi_post, method = "posterior_epred", prob = 0.9, dpar = "hu")
CE90_pp = conditional_effects(m_Takahashi_post, method = "posterior_predict", prob = 0.9)

wig = (CE50_mu)[[1]]$wiggleN_Takahashi
vid = (CE50_mu)[[1]]$video_pce_IN
mu = (CE50_mu)[[1]]$estimate

ppwig = (CE50_pp)[[1]]$wiggleN_Takahashi
...
plotdat_Takahashi = data.frame(wig = wig, vid = vid, 
                     mu = mu, 
                    # lwl = lwl, upl = upl,
...
prediction_plot = ggplot() + 
  ...
  geom_ribbon(data = plotdat_Takahashi, aes(x = ppwig, ymin = pp50lwl, ymax = pp50upl), alpha = 0.2, fill = "grey51") +
  geom_ribbon(data = plotdat_Takahashi, aes(x = ppwig, ymin = pp80lwl, ymax = pp80upl), alpha = 0.2, fill = "grey51") +
  geom_ribbon(data = plotdat_Takahashi, aes(x = ppwig, ymin = pp90lwl, ymax = pp90upl), alpha = 0.2, fill = "grey51", col = "black")+ 
# epred (mu)
...
geom_ribbon(data = plotdat_Takahashi, aes(x = wig, ymin = lwl90, ymax = upl90), alpha = 0.3, fill = "#440154FF") +
  geom_line(data = plotdat_Takahashi, aes(x = wig, y = mu), linewidth = 1, color = "black") +
 # gg_theme() +
  gg_theme()+
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi_prediction_plot.png"), prediction_plot, width = 6, height = 4, dpi = 300)
...
models_Takahashi_train <- list()
predict_on_test_Takahashi <- list()
ggplots_Takahashi <- list()
ggplots2_Takahashi <- list()
...
models_Takahashi_train[[ids]] <- 
    brm(bf(video_pce_IN ~ wiggleN_Takahashi, hu ~ wiggleN_Takahashi), 
        data = train_data,
        family = hurdle_negbinomial(link = "identity", link_shape = "log", link_hu = "logit"),
        iter = 2000, warmup = 1000, chains = 4, cores = cores,
        prior = prior,
        sample_prior = T, 
        file = paste("./brms/Takahashi/m_Takahashi_train_", ids))

# predict on excluded individual
predict_on_test = as.data.frame(predict(models_Takahashi_train[[ids]], 
                                newdata = test_data))
...
predict_on_test_Takahashi[[ids]] = cbind(test_data, predict_on_test)

#n_distinct(predict_on_test_Takahashi[[ids]]$id)
#unique(predict_on_test_Takahashi[[ids]]$id)

ggplots_Takahashi[[ids]] = ggplot(data = 
         predict_on_test_Takahashi[[ids]],
         aes(x = video_pce_IN, y = Estimate)) + 
         geom_point() + 
         theme_bw() + 
         xlim(0,126) +
         labs(subtitle = paste0("Takahashi et al. 2004. ",
                        "Training data (N) = ", n_distinct(train_data$id),
                        ". Test ID = ", unique(predict_on_test_Takahashi[[ids]]$id)))

ggplots_Takahashi[[ids]]

# Save as PNG

ggsave(paste0("./plots/brms/Takahashi/Takahashi_prediction_observed_", ids, ".png"), ggplots_Takahashi[[ids]], width = 7, height = 5, dpi = 300)

# Cumulative PCE 
# Video observed cumulative PCE
predict_on_test_Takahashi[[ids]]$video_cumulativePCE

# Cumulative wiggles
predict_on_test_Takahashi[[ids]]$cumulativeEstimate = cumsum(predict_on_test_Takahashi[[ids]]$Estimate)
predict_on_test_Takahashi[[ids]]$cumulativeQ2.5 = cumsum(predict_on_test_Takahashi[[ids]]$Q2.5)
predict_on_test_Takahashi[[ids]]$cumulativeQ97.5 = cumsum(predict_on_test_Takahashi[[ids]]$Q97.5)

ggplots2_Takahashi[[ids]] = predict_on_test_Takahashi[[ids]] %>%
         ggplot() + 
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi/Takahashi_cumulative_pred_obs_", ids, ".png"), ggplots2_Takahashi[[ids]], width = 7, height = 5, dpi = 300)

}

######

predict_test_Takahashi = bind_rows(predict_on_test_Takahashi)
dim(predict_test_Takahashi)
names(predict_test_Takahashi)


Density_est <- predict_test_Takahashi %>% 
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi_LOGOCV_prediction_plot.png"), LOGOCV_preds, width = 6, height = 4, dpi = 300)
...
# Save as PNG
ggsave(paste0("./plots/brms/Takahashi_LOGOCV_cum_prediction_plot.png"), LOGOCV_cum_preds, width = 6, height = 4, dpi = 300)
