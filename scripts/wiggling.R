...
# 2211 is the "total" in the raw

# processing file

# you can lose PCE when slicing the first 30 rows (to remove initial videos, which are not always present) 
temp = df1 %>% 
  drop_na(vid_time) %>%
  slice(31:n())

# 2210 is the "total" after processing

# dive analysis file
# you can lose PCE when slicing to make sure you are dealing with complete dives
dat = dat %>% arrange(date.time)
index_start = min(which(dat$depth < depth.threshold))
index_end = max(which(dat$depth < depth.threshold))

# 2209 is the "total" after dive analysis

# The sum after wiggling is only
# [1] 2192

# THE EXCLUDED DIVES ARE ALL BELOW 3 M, WHICH WAS THE REFERENCE FOR DIVEMOVE!
# THERE MIGHT BE SOME PCE AT DEPTHS SHALLOWER THAN 3 M IN THE FILE,
# BECAUSE DIVEMOVE DOES NOT USE EXPLICIT THRESHOLDS.
# BUT NO PCE DEEPER THAN 3 M APPEARS TO BE IN THE EXCLUDED DATA

# MOST PCE happening in X (In-DIVE) will be removed 

# NOW WHAT ABOUT VIDEO? (NO VID)?

# One can look at dives 50, 60, 80 for example. 
# These dives have PCE, but they are labelled as NOVIDEO. They contain both 'video' and NA
# sections. So they are correctly labelled as novideo ("some" of the dive contain NA for video)
# and they correctly include PCE (for the portion of the dive where there IS video.)

# One can (choose to) include the portion of the dive that has video. This would, however, bias the
# 'total PCE per dive" summaries since the penguin did part of the dive without 
# any video (though this bias will be very small, overall)
...
w = w %>% select(date.time, depth, PCE_1hz)
head(w)
dd = dd %>% select(date.time, depth, PCE_1hz)
head(dd)

dim(dd)  # has ALL
dim(w)

# Which rows is missing in w?
tst = setdiff(dd, w)
tst  # OK - these are all shallower than 3 m! 
dim(tst)
17+2192

# OKtst# OK, so what are included in w?
tst2 = setdiff(w, tst)
tst2  # OK - these are all shallower than 3 m! 
...
view(wiggle_summary_all_data)

N = result_all %>%
  filter(video.level == 'novideo')
view(N)

# look at dive 50 and 60:

d60 = d %>% 
  filter(dive.nr == 80)
view(d60)

Y = wiggle_summary_all_data %>%
  filter(video.level == 'video') %>%
  filter(id == "2022_01_22_AC2110_DI06")
view(Y)
...
result_df %>% 
  ggplot() + aes(x = maxdep, y = id, fill = stat(x)) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_viridis_c(name = "Max depth (m)", option = "C") +
  theme_bw() + 
  #  theme(axis.text.y = element_blank(),
  #        axis.ticks.y = element_blank()) + 
  geom_hline(yintercept = 14.5, linetype = "dashed", color = "red") +
...
# 1 =  number that each bout should 'advace' with if yes
# 0 =  number that each bout should 'advace' with if no 
...
result_df = result_df %>%
  mutate(foraging_dive = ifelse(video_pce_IN == 0, "nonforaging", "foraging"))
...
result_df = result_df %>%
  group_by(id, bout) %>%
  mutate(foraging_bout = ifelse(mean(foraging_dive == "foraging") < 0.5, "nonforaging", "foraging")) %>%
  ungroup()


# Calculate how many foraging and nonforaging dives there are (video observations)
divetypeN_video <- result_df %>%  
  group_by(id) %>%
  summarise(foraging_video = sum(foraging_dive == "foraging"),
            nonforaging_video = sum(foraging_dive == "nonforaging"),
            total_dives = sum(foraging_video + nonforaging_video))

divetypeN_video

# Calculate how many foraging and nonforaging dives were predicted by each wiggle analysis
divetype_wiggles <- result_df %>%
  #     dplyr::filter(maxdep > 10) %>%
  group_by(id) %>%
  mutate(
    pred_wiggleN_Halsey_05m = ifelse(wiggleN_Halsey_05m > 0, "foraging", "nonforaging"),
    pred_wiggleN_Halsey_2m = ifelse(wiggleN_Halsey_2m > 0, "foraging", "nonforaging"),
    pred_wiggleN_Simeone = ifelse(wiggleN_Simeone > 0, "foraging", "nonforaging"),
    pred_wiggleN_Takahashi = ifelse(wiggleN_Takahashi > 1, "foraging", "nonforaging")) %>%
  summarise(
    foraging_Halsey_05m = sum(pred_wiggleN_Halsey_05m == "foraging"),
    nonforaging_Halsey_05m = sum(pred_wiggleN_Halsey_05m == "nonforaging"),
    
    foraging_Halsey_2m = sum(pred_wiggleN_Halsey_2m == "foraging"),
    nonforaging_Halsey_2m = sum(pred_wiggleN_Halsey_2m == "nonforaging"),
    
    foraging_Simeone = sum(pred_wiggleN_Simeone == "foraging"),
    nonforaging_Simeone = sum(pred_wiggleN_Simeone == "nonforaging"),
    
    foraging_Takahashi = sum(pred_wiggleN_Takahashi == "foraging"),
    nonforaging_Takahashi = sum(pred_wiggleN_Takahashi  == "nonforaging"))
...
divetype_summary = all_divetype  %>%
  pivot_longer(cols = starts_with(c("foraging", "nonforaging")),
...
#-------------------------------------------------------------------------------------
# How does max depth correlate with odds of foraging / nonforaging dives with wiggles?
#-------------------------------------------------------------------------------------
wiggledepth <- result_df %>%
  ...
  mutate(
    ...
    pred_wiggleN_Halsey_2m = ifelse(wiggleN_Halsey_2m > 0, "foraging", "nonforaging"),
    pred_wiggleN_Simeone = ifelse(wiggleN_Simeone > 0, "foraging", "nonforaging"),
    pred_wiggleN_Takahashi = ifelse(wiggleN_Takahashi > 1, "foraging", "nonforaging"))  # exclude the '1' compulsory wiggle per dive
...
wiggledepth <- wiggledepth %>%
  mutate(Halsey_05m = case_when(
    ...
    foraging_dive == "foraging" & pred_wiggleN_Halsey_05m == "nonforaging" ~ "false_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Halsey_05m == "nonforaging" ~ "true_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Halsey_05m == "foraging" ~ "false_pos"))  %>%
  mutate(Halsey_2m = case_when(
    foraging_dive == "foraging" & pred_wiggleN_Halsey_2m == "foraging" ~ "true_pos",
    foraging_dive == "foraging" & pred_wiggleN_Halsey_2m == "nonforaging" ~ "false_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Halsey_2m == "nonforaging" ~ "true_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Halsey_2m == "foraging" ~ "false_pos")) %>%
  
  mutate(Simeone = case_when(
    foraging_dive == "foraging" & pred_wiggleN_Simeone == "foraging" ~ "true_pos",
    foraging_dive == "foraging" & pred_wiggleN_Simeone == "nonforaging" ~ "false_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Simeone == "nonforaging" ~ "true_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Simeone == "foraging" ~ "false_pos"))  %>%
  
  mutate(Takahashi = case_when(
    foraging_dive == "foraging" & pred_wiggleN_Takahashi == "foraging" ~ "true_pos",
    foraging_dive == "foraging" & pred_wiggleN_Takahashi == "nonforaging" ~ "false_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Takahashi == "nonforaging" ~ "true_neg",
    foraging_dive == "nonforaging" & pred_wiggleN_Takahashi == "foraging" ~ "false_pos"))
...
#-----------------------------------------------------------------------
# Boxplot: wiggles and video dive types (foraging or nonforaging dive)
#-----------------------------------------------------------------------
...
# #  filter(!(is.na(bottdist) | bottdist == 0) & foraging_dive != "nonforaging") %>%
...
# Save as PNG
ggsave("./plots/Figure4_example.png", Figure5_example, width = 8, height = 9, dpi = 1000)
