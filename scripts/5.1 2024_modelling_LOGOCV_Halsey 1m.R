...
#--------------------------------------------
brms::get_prior(bf(
  video_pce_IN ~ wiggleN_Halsey_1m, hu ~ wiggleN_Halsey_1m), 
  data = result_df,
...
prior <- c(
  ...
  set_prior("normal(1, 5)", class = "b"),                      # Prior for regression coefficients (mu_wiggleN_Halsey_1m)
  set_prior("normal(0, 2)", class = "b", dpar = "hu"),         # Prior for regression coefficients (hu_wiggleN_Halsey_1m)
...
m_Halsey_1m_prior <- brm(bf(
  ...
  prior = prior,
  sample_prior = "only", 
  file = "./brms/Halsey_1m/m_Halsey_1m_prior")

m_Halsey_1m_post <- brm(bf(
  video_pce_IN ~ wiggleN_Halsey_1m, hu ~ wiggleN_Halsey_1m), 
  data = result_df,
  family = hurdle_negbinomial(link = "identity", link_shape = "log", link_hu = "logit"),
  iter = 2000, warmup = 1000, chains = 4, cores = cores,
  prior = prior,
  sample_prior = T, 
  file = "./brms/Halsey_1m/m_Halsey_1m_post")

# Compare the empirical distribution of the data y to the distributions of simulated/replicated data yrep 
# from the posterior predictive distribution. 

# bayesplot::ppc_dens_overlay(y = result_df$video_pce_IN, yrep = m_Halsey_1m_pp[1:100, ]) +
#   labs(title = "Prior Predictive Distribution",
#        x = "video_pce_IN",
#        y = "Density")
performance::check_predictions(m_Halsey_1m_post) +  
...
m_Halsey_1m_pp <- posterior_predict(m_Halsey_1m_post)
...
# Save as PNG
ggsave(paste0("./plots/brms/Halsey_1m_ppd.png"), PPD, width = 5, height = 5, dpi = 300)

loo_Halsey_1m_post = loo(m_Halsey_1m_post)
loo_Halsey_1m_post

#-----------------------------------------------------
# Extract parameter draws for the prior-only model
#-----------------------------------------------------
prior_draws <- as_draws_df(m_Halsey_1m_prior)

# Extract parameter draws for the posterior model
posterior_draws <- as_draws_df(m_Halsey_1m_post)
...
draws$Parameter <- recode(draws$Parameter,
                          ...
                          "b_wiggleN_Halsey_1m" = "mu_wiggleN_Halsey_1m",
                          "b_hu_Intercept" = "hu_Intercept",
                          "b_hu_wiggleN_Halsey_1m" = "hu_wiggleN_Halsey_1m",
                          "shape" = "Shape")
...
# Save as PNG
ggsave(paste0("./plots/brms/Halsey_1m_prior_post.png"), PPD2, width = 5, height = 5, dpi = 300)

result_df %>%
  ungroup() %>% 
  modelr::data_grid(wiggleN_Halsey_1m = modelr::seq_range(wiggleN_Halsey_1m, n = 100)) %>%
  add_predicted_draws(m_Halsey_1m_post) %>%
  ggplot(aes(x = wiggleN_Halsey_1m, y = video_pce_IN)) +
  stat_lineribbon(aes(y = .prediction), .width = c(.90, .80, .50), alpha = 0.9) +
  geom_point(data = result_df) +
  scale_fill_brewer(palette = "Blues") +
  scale_color_brewer(palette = "Blues") + 
  labs(y = "Video PCE", x = 'Wiggle_Halsey 1m') +
...
# CE_mu_m_Halsey_1m = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.9)
# CE_mu_m_Halsey_1m
# CE_mu_h_m_Halsey_1m = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.9, dpar = "hu")
# CE_mu_h_m_Halsey_1m
...
# CE_pi_m_Halsey_1m = conditional_effects(m_Halsey_1m_post, method = "posterior_predict", prob = 0.9)
# CE_pi_m_Halsey_1m
...
# Extract components:
# --------------------------
CE50_mu = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.5)
CE50_mu_h = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.5, dpar = "hu")
CE50_pp = conditional_effects(m_Halsey_1m_post, method = "posterior_predict", prob = 0.5)

CE80_mu = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.8)
CE80_mu_h = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.8, dpar = "hu")
CE80_pp = conditional_effects(m_Halsey_1m_post, method = "posterior_predict", prob = 0.8)

CE90_mu = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.9)
CE90_mu_h = conditional_effects(m_Halsey_1m_post, method = "posterior_epred", prob = 0.9, dpar = "hu")
CE90_pp = conditional_effects(m_Halsey_1m_post, method = "posterior_predict", prob = 0.9)

wig = (CE50_mu)[[1]]$wiggleN_Halsey_1m
vid = (CE50_mu)[[1]]$video_pce_IN
mu = (CE50_mu)[[1]]$estimate

ppwig = (CE50_pp)[[1]]$wiggleN_Halsey_1m
...
plotdat_Halsey_1m = data.frame(wig = wig, vid = vid, 
                     mu = mu, 
                    # lwl = lwl, upl = upl,
...
prediction_plot = ggplot() + 
  ...
  geom_ribbon(data = plotdat_Halsey_1m, aes(x = ppwig, ymin = pp50lwl, ymax = pp50upl), alpha = 0.2, fill = "grey51") +
  geom_ribbon(data = plotdat_Halsey_1m, aes(x = ppwig, ymin = pp80lwl, ymax = pp80upl), alpha = 0.2, fill = "grey51") +
  geom_ribbon(data = plotdat_Halsey_1m, aes(x = ppwig, ymin = pp90lwl, ymax = pp90upl), alpha = 0.2, fill = "grey51", col = "black")+ 
# epred (mu)
...
geom_ribbon(data = plotdat_Halsey_1m, aes(x = wig, ymin = lwl90, ymax = upl90), alpha = 0.3, fill = "#440154FF") +
  geom_line(data = plotdat_Halsey_1m, aes(x = wig, y = mu), linewidth = 1, color = "black") +
 # gg_theme() +
  gg_theme()+
...
# Save as PNG
ggsave(paste0("./plots/brms/Halsey_1m_prediction_plot.png"), prediction_plot, width = 6, height = 4, dpi = 300)
...
models_Halsey_1m_train <- list()
predict_on_test_Halsey_1m <- list()
ggplots_Halsey_1m <- list()
ggplots2_Halsey_1m <- list()
...
models_Halsey_1m_train[[ids]] <- 
    brm(bf(video_pce_IN ~ wiggleN_Halsey_1m, hu ~ wiggleN_Halsey_1m), 
        data = train_data,
        family = hurdle_negbinomial(link = "identity", link_shape = "log", link_hu = "logit"),
        iter = 2000, warmup = 1000, chains = 4, cores = cores,
        prior = prior,
        sample_prior = T, 
        file = paste("./brms/Halsey_1m/m_Halsey_1m_train_", ids))

# predict on excluded individual
predict_on_test = as.data.frame(predict(models_Halsey_1m_train[[ids]], 
                                newdata = test_data))
...
predict_on_test_Halsey_1m[[ids]] = cbind(test_data, predict_on_test)

#n_distinct(predict_on_test_Halsey_1m[[ids]]$id)
#unique(predict_on_test_Halsey_1m[[ids]]$id)

ggplots_Halsey_1m[[ids]] = ggplot(data = 
         predict_on_test_Halsey_1m[[ids]],
...
ggplots_Halsey_1m[[ids]]

# Save as PNG

ggsave(paste0("./plots/brms/Halsey_1m/Halsey_1m_prediction_observed_", ids, ".png"), ggplots_Halsey_1m[[ids]], width = 7, height = 5, dpi = 300)

# Cumulative PCE 
# Video observed cumulative PCE
predict_on_test_Halsey_1m[[ids]]$video_cumulativePCE

# Cumulative wiggles
predict_on_test_Halsey_1m[[ids]]$cumulativeEstimate = cumsum(predict_on_test_Halsey_1m[[ids]]$Estimate)
predict_on_test_Halsey_1m[[ids]]$cumulativeQ2.5 = cumsum(predict_on_test_Halsey_1m[[ids]]$Q2.5)
predict_on_test_Halsey_1m[[ids]]$cumulativeQ97.5 = cumsum(predict_on_test_Halsey_1m[[ids]]$Q97.5)

ggplots2_Halsey_1m[[ids]] = predict_on_test_Halsey_1m[[ids]] %>%
         ggplot() + 
...
# Save as PNG
ggsave(paste0("./plots/brms/Halsey_1m/Halsey_1m_cumulative_pred_obs_", ids, ".png"), ggplots2_Halsey_1m[[ids]], width = 7, height = 5, dpi = 300)

}

######

predict_test_Halsey_1m = bind_rows(predict_on_test_Halsey_1m)
dim(predict_test_Halsey_1m)
names(predict_test_Halsey_1m)


Density_est <- predict_test_Halsey_1m %>% 
...
# Save as PNG
ggsave(paste0("./plots/brms/Halsey_1m_LOGOCV_prediction_plot.png"), LOGOCV_preds, width = 6, height = 4, dpi = 300)

#------------------------------------------------------
# plot LOGO-CV cumulative predictions of all penguins
#------------------------------------------------------
LOGOCV_cum_preds =
  predict_test_Halsey_1m  %>%
  arrange(dive.nr) %>%
  ggplot() + 
