...
#----------------------------------
result_all = readRDS("./output/result_all_3m_JEMBErevision.rds") 
subdat_Halsey_1m = readRDS("./output/subdat_Halsey_1m_3m.rds")
subdat_Halsey_2m = readRDS("./output/subdat_Halsey_2m_3m.rds")
subdat_Simeone = readRDS("./output/subdat_Simeone_3m_JEMBErevision.rds")
...
threshold <- quantile(subdat_r$depth_ratio, probs = 0.1, na.rm = TRUE)
...
relative_depth_plot = 
  ...
  labs(x = "Relative depth",
    y = "Count") + 
#    title = "Histogram of prey capture depth relative to maximum dive depths (Highlighting Top 90%)") +
  gg_theme() + 
...
ggsave("./supplement/relative_depth_plot.png", relative_depth_plot, width = 9, height = 6, dpi = 600)
...
#    wiggle_sum_Simeone = sum(wiggleN_Simeone),
    wiggle_sum_Simeone = sum(wiggle_SW_bottom),
#    all_wiggle_sum_Simeone = sum(SW_all_wig_in_s_e),
...
maxdep_id_all = result_all %>% 
  ...
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01, bandwidth = 2.2) +   # bandwidth controls the smoothness
...
#--Supplement ------------------------------------------------------------------------------------------
# Create a foraging_dive column based on video observations (is this a foraging dive?)
# result_df_strict = result_df %>%
#              mutate(foraging_dive = ifelse(maxdep >= 10 & video_pce_IN >=5, "foraging", "non-foraging"))
# 
# saveRDS(result_df_strict, "./output/supplement_result_df_strict.rds")
#---------------------------------------------------------------------------------------------
...
divetype_wiggles <- result_df %>%
     #     dplyr::filter(maxdep > 10) %>%
          ...
          mutate(
            ...
            pred_wiggleN_Simeone = ifelse(wiggle_SW_bottom> 0, "foraging", "non-foraging"),
...
#  ggtitle("Halsey et al. 2007 (1m)") + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 6, vjust = -2),
...
Density_map_wiggleN_Simeone <- result_df %>% 
   dplyr::select(id, video_pce_IN, wiggle_SW_bottom) %>% 
   mutate(Density = get_Density(video_pce_IN, wiggle_SW_bottom, n = 100))
...
fig5_bottdist = Density_map_bottdist %>% 
  ...
  annotate("text", 
           x = Inf, y = Inf, 
           label = "Bottom distance", size = 3.4, 
...
fig5_Simeone = Density_map_wiggleN_Simeone %>% 
  ggplot(aes(x = wiggle_SW_bottom, y = video_pce_IN, color = Density)) +
  #geom_point(alpha = 0.3) +
 ...
 # scale_x_continuous(limits = c(-0.1, 16)) +
  annotate("text",
           x = Inf, y = Inf, 
           label = "Relative vertical velocity wiggles (0.35m)", size = 3.4,  
           hjust = 1.05, vjust = 1.6)+
...
fig5_Takahashi = Density_map_wiggleN_Takahashi %>% 
  ...
  annotate("text",
           x = Inf, y = Inf, 
           label = "Single changepoint wiggles", size = 3.4, 
           hjust = 1.1, vjust = 1.6)+
...
fig5_Halsey_1m = Density_map_wiggleN_Halsey_1m %>% 
  ...
  annotate("text",
           x = Inf, y = Inf, 
           label = "Three-fold changepoint wiggles (1m)", size = 3.4, 
           hjust = 1.1, vjust = 1.6)+ 
  theme(#legend.text=element_text(size = 8),
    #           legend.title=element_text(size = 10),
    legend.key.size=unit(0.8, "lines"))
...
fig5_Halsey_2m = Density_map_wiggleN_Halsey_2m %>% 
  ...
  annotate("text",
           x = Inf, y = Inf, 
           label = "Three-fold changepoint wiggles (2m)", size = 3.4, 
           hjust = 1.1, vjust = 1.6) + 
   theme(#legend.text=element_text(size = 8),
          #           legend.title=element_text(size = 10),
                     legend.key.size=unit(0.8, "lines"))
...
fig5_Halsey_1m_ind = Density_map_wiggleN_Halsey_1m %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")+
  ylab("Video prey captures") +
  xlab("Three-fold changepoint wiggles (1m) (Halsey et al. 2007)") +
  scale_y_continuous(limits = c(0,80), breaks = seq(0,100, by = 25))+
  scale_x_continuous(limits = c(-0.1, 16)) +
  theme(legend.position = "none")+ 
  theme(strip.background = element_blank(),
...
fig5_Halsey_2m_ind = Density_map_wiggleN_Halsey_2m %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")+
  ylab("Video prey captures") +
  xlab("Three-fold changepoint wiggles (2m) (Halsey et al. 2007)") +
  scale_y_continuous(limits = c(0,80), breaks = seq(0,100, by = 25))+
  scale_x_continuous(limits = c(-0.1, 16)) +
  theme(legend.position = "none")+ 
  theme(strip.background = element_blank(),
...
fig5_Takahashi_ind = Density_map_wiggleN_Takahashi %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")+
  ylab("Video prey captures") +
  xlab("Single changepoint wiggles (Takahashi et al. 2004)") +
  scale_y_continuous(limits = c(0,80), breaks = seq(0,100, by = 25))+
  scale_x_continuous(limits = c(-0.1, 16)) +
  theme(legend.position = "none")+ 
  theme(strip.background = element_blank(),
...
fig5_Simeone_ind = Density_map_wiggleN_Simeone %>% 
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black")+
  ylab("Video prey captures") +
  xlab("Relative vertical velocity wiggles (0.35m) (Simeone and Wilson 2003)") +
  scale_y_continuous(limits = c(0,80), breaks = seq(0,100, by = 25))+
#  scale_x_continuous(limits = c(-0.1, 16)) +
  theme(legend.position = "none")+ 
  theme(strip.background = element_blank(),
...
result_df = result_df %>% 
  ...
  mutate(video_cumulativePCE = cumsum(video_pce_IN),
         ...
         cumulativeSimeone = cumsum(wiggle_SW_bottom),
...
# Create left-aligned, slightly lower labels
col_A <- wrap_elements(grid::textGrob("A",
                                      x = 0,           # far left
                                      y = 0.3,         # slightly down from top (default 1)
                                      hjust = 0,       # left-align text
                                      vjust = 0,       # top-aligned relative to y position
                                      gp = grid::gpar(cex = 1)))

col_B <- wrap_elements(grid::textGrob("B",
                                      x = 0,
                                      y = 0.3,
                                      hjust = 0,
                                      vjust = 0,
...
# Combine into labeled layout
PCE_cumulative <- (col_A + col_B) / wrap_plots(plots, ncol = 2) +
...
#   dplyr::filter(date.time < '2023-01-14 20:04:10') 
#  dplyr::filter(date.time > '2023-01-14 20:21:25') %>%
#   dplyr::filter(date.time < '2023-01-14 20:24:00')
...
# Save as PNG
ggsave("./supplement/Supp_fig_pce_hist.png", PC_dive_id2, width = 6, height = 8, dpi = 1200)
...
result_df = result_df %>% 
  ...
  mutate(CPUE_video = sum(video_pce_IN) / sum(botttim),
         ...
         CPUE_Simeone = sum(wiggle_SW_bottom) / sum(botttim),
...
cpue_long$CPUE_metric <- factor(cpue_long$CPUE_metric, 
                                ...
                                labels = c("CPUE_video" = "Video\nobservations",
                                           "CPUE_Simeone"  = "Relative vertical velocity\nwiggles (0.35m)",
                                           "CPUE_Takahashi" = "Single changepoint\nwiggles",
                                           "CPUE_Halsey_1m" = "Three-fold changepoint\nwiggles (1m)",
                                           "CPUE_Halsey_2m" = "Three-fold changepoint\nwiggles (2m)",
                                           "CPUE_bottdist" = "Bottom\ndistance"))
...
cpue_long_scaled$CPUE_metric <- factor(cpue_long_scaled$CPUE_metric, 
                                       ...
                                       labels = c("scaled_CPUE_video" = "Video\nobservations",
                                                  "scaled_CPUE_Simeone"  = "Relative vertical velocity\nwiggles (0.35m)",
                                                  "scaled_CPUE_Takahashi" = "Single changepoint\nwiggles",
                                                  "scaled_CPUE_Halsey_1m" = "Three-fold changepoint\nwiggles (1m)",
                                                  "scaled_CPUE_Halsey_2m" = "Three-fold changepoint\nwiggles (2m)",
                                                  "scaled_CPUE_bottdist" = "Bottom\ndistance"))
...
#------------------------
# CPUE Regression figures
#------------------------
mcpue_rvv = lm(CPUE_video ~ CPUE_Simeone, result_df)
summary(mcpue_rvv)
mcpue_1m = lm(CPUE_video ~ CPUE_Halsey_1m, result_df)
summary(mcpue_1m)
mcpue_2m = lm(CPUE_video ~ CPUE_Halsey_2m, result_df)
summary(mcpue_2m)
mcpue_single = lm(CPUE_video ~ CPUE_Takahashi, result_df)
summary(mcpue_single)
mcpue_bottom = lm(CPUE_video ~ CPUE_bottdist, result_df)
summary(mcpue_bottom)


cpue_rvv = ggplot(data = result_df,
                   aes(x = CPUE_Simeone, 
                       y = CPUE_video)) + 
              geom_point(fill = "navy", shape = 21, size = 2) + 
              gg_theme() + 
      ylab("CPUE: video observations") +
      xlab("CPUE: Relative vertical velocity wiggles (0.35m)") + 
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text", x = Inf, y = Inf, label =  paste0("R^2 == ", 0.24), hjust = 1.5, vjust = 2, parse = TRUE)
cpue_rvv 

cpue_1m = ggplot(data = result_df,
       aes(x = CPUE_Halsey_1m, 
           y = CPUE_video)) + 
  geom_point(fill = "navy", shape = 21, size = 2) + 
  gg_theme() + 
  ylab("CPUE: video observations") +
  xlab("CPUE: Three-fold changepoint wiggles (1m)") + 
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text", x = Inf, y = Inf, label =  paste0("R^2 == ", 0.49), hjust = 1.5, vjust = 2, parse = TRUE)
cpue_1m

cpue_2m = ggplot(data = result_df,
       aes(x = CPUE_Halsey_2m, 
           y = CPUE_video)) + 
  geom_point(fill = "navy", shape = 21, size = 2)+ 
  gg_theme() + 
  ylab("CPUE: video observations") +
  xlab("CPUE: Three-fold changepoint wiggles (2m)") + 
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text", x = Inf, y = Inf, label =  paste0("R^2 == ", 0.19), hjust = 1.5, vjust = 2, parse = TRUE)
cpue_2m

cpue_single = ggplot(data = result_df,
       aes(x = CPUE_Takahashi, 
           y = CPUE_video)) + 
  geom_point(fill = "navy", shape = 21, size = 2) + 
  gg_theme() + 
  ylab("CPUE: video observations") +
  xlab("CPUE: Single changepoint wiggles") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text", x = Inf, y = Inf, label =  paste0("R^2 == ", 0.008), hjust = 1.5, vjust = 2, parse = TRUE)

cpue_single


cpue_bottom = ggplot(data = result_df,
       aes(x = CPUE_bottdist, 
           y = CPUE_video)) + 
  geom_point(fill = "navy", shape = 21, size = 2) + 
  gg_theme() + 
  ylab("CPUE: video observations") +
  xlab("CPUE: Bottom distance")  +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
  annotate("text", x = Inf, y = Inf, label =  paste0("R^2 == ", 0.10), hjust = 1.5, vjust = 2, parse = TRUE)
cpue_bottom

cpue_video = ggplot(data = result_df,
                     aes(x = CPUE_video, 
                         y = CPUE_video)) + 
  geom_point(fill = "navy", shape = 21, size = 2) + 
  gg_theme() + 
  ylab("CPUE: video observations") +
  xlab("CPUE: video observations") +
  geom_smooth(method = "lm", se = FALSE, color = "black") 

cpue_regress = cpue_video + cpue_rvv + cpue_single + cpue_1m + cpue_2m + cpue_bottom 

# Save as PNG
ggsave("./supplement/cpue_regress.png", cpue_regress, width = 12, height = 8, dpi = 600)
